...
class NoiseScheduleVP:
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == 'linear':
            ...
            log_alpha_fn = lambda s: ops.log(ops.cos((s + self.cosine_s) / (1. + self.cosine_s) * math.pi / 2.))
    ...
    def marginal_std(self, t):
        ...
        """
        return ops.sqrt(1. - ops.exp(2. * self.marginal_log_mean_coeff(t)))
    ...
    def inverse_lambda(self, lamb):
        ...
        """
        if self.schedule == 'linear':
            tmp = 2. * (self.beta_1 - self.beta_0) * ops.logaddexp(-2. * lamb, ops.zeros((1,)).to(lamb.dtype))
            Delta = self.beta_0 ** 2 + tmp
            return tmp / (ops.sqrt(Delta) + self.beta_0) / (self.beta_1 - self.beta_0)
        else:
            log_alpha = -0.5 * ops.logaddexp(-2. * lamb, ops.zeros((1,)).to(lamb.dtype))
            t_fn = lambda log_alpha_t: ops.arccos(ops.exp(log_alpha_t + self.cosine_log_alpha_0)) * 2. * (
                    1. + self.cosine_s) / math.pi - self.cosine_s
            t = t_fn(log_alpha)
            return t
...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1.,
    ...
    def get_model_input_time(t_continuous):
        ...
        if time_input_type == '0':
            ...
            return 1000. * ops.maximum(t_continuous - 1. / total_N, ops.zeros_like(t_continuous).to(t_continuous.dtype))
    ...
    def cond_fn(x, t_discrete, y):
        ...
        #     return classifier_scale * ops.autograd.grad(selected.sum(), x_in)[0]
        return classifier_fn(x, t_discrete, y)
...
class DPM_Solver:
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N,):
        ...
        Returns:
            A pyops tensor of the time steps, with the shape (N + 1,).
        """
        if skip_type == 'logSNR':
            lambda_T = self.noise_schedule.marginal_lambda(Tensor(t_T))
            lambda_0 = self.noise_schedule.marginal_lambda(Tensor(t_0))
            logSNR_steps = ops.linspace(lambda_T, lambda_0, N + 1)
            return self.noise_schedule.inverse_lambda(logSNR_steps)
        elif skip_type == 'time_uniform':
            return ops.linspace(t_T, t_0, N + 1)
        elif skip_type == 'time_quadratic':
            t = ops.linspace(t_0, t_T, 10000000)
            quadratic_t = ops.sqrt(t)
            quadratic_steps = ops.linspace(quadratic_t[0], quadratic_t[-1], N + 1)
            return ops.flip(
                ops.cat([t[ops.searchsorted(quadratic_t, quadratic_steps)[:-1]], t_T * ops.ones((1,))],
                        axis=0), dims=[0])
        else:
            raise ValueError(
                "Unsupported skip_type {}, need to be 'logSNR' or 'time_uniform' or 'time_quadratic'".format(skip_type))

    def get_time_steps_for_dpm_solver_fast(self, t_T, t_0, steps):
        """
        ...
        Returns:
            ...
            timesteps: A pyops tensor of the time steps, with the shape of (K + 1,).
        """
        K = steps // 3 + 1
        if steps % 3 == 0:
            orders = [3, ] * (K - 2) + [2, 1]
            ...
            orders = [3, ] * (K - 1) + [2]
        timesteps = self.get_time_steps('logSNR', t_T, t_0, K)
    ...
    def dpm_solver_first_update(self, x, s, t, return_noise=False):
        ...
        Args:
            x: A pyops tensor. The initial value at time `s`.
            s: A pyops tensor. The starting time, with the shape (x.shape[0],).
            t: A pyops tensor. The ending time, with the shape (x.shape[0],).
            return_noise: A `bool`. If true, also return the predicted noise at time `s`.
        Returns:
            x_t: A pyops tensor. The approximated solution at time `t`.
        """
        ...
        x_t = (
                ops.exp(log_alpha_t - log_alpha_s)[(...,) + (None,) * dims] * x
    ...
    def dpm_solver_second_update(self, x, s, t, r1=0.5, noise_s=None, return_noise=False):
        ...
        Args:
            ...
            noise_s: A pyops tensor. The predicted noise at time `s`.
        ...
        if noise_s is None:
            noise_s = self.model_fn(x, s)
        x_s1 = (
                ops.exp(log_alpha_s1 - log_alpha_s)[(...,) + (None,) * dims] * x
    ...
    def dpm_solver_third_update(self, x, s, t, r1=1. / 3., r2=2. / 3., noise_s=None, noise_s1=None, noise_s2=None):
        ...
        Args:
            ...
            noise_s: A pyops tensor. The predicted noise at time `s`.
                If `noise_s` is None, we compute the predicted noise by `x` and `s`; otherwise we directly use it.
            noise_s1: A pyops tensor. The predicted noise at time `s1` (the intermediate time given by `r1`).
        ...
        phi_1 = ops.expm1(h)
        phi_22 = ops.expm1(r2 * h) / (r2 * h) - 1.
        phi_2 = ops.expm1(h) / h - 1.
        start_time=time.time()
        if noise_s is None:
            noise_s = self.model_fn(x, s)
        if noise_s1 is None:
        ...
        if noise_s2 is None:
            x_s2 = (
                    ops.exp(log_alpha_s2 - log_alpha_s)[(...,) + (None,) * dims] * x
    ...
    def dpm_solver_adaptive(self, x, order, t_T, t_0, h_init=0.05, atol=0.0078, rtol=0.05, theta=0.9, t_err=1e-5):
        ...
        Args:
            x: A pyops tensor. The initial value at time `t_T`.
        ...
        Returns:
            x_0: A pyops tensor. The approximated solution at time `t_0`.

        [1] A. Jolicoeur-Martineau, K. Li, R. Piché-Taillefer, T. Kachman, and I. Mitliagkas, "Gotta go fast when generating data with score-based models," arXiv preprint arXiv:2105.14080, 2021.
        """
        ns = self.noise_schedule
        s = t_T * ops.ones((x.shape[0],)).to(x.dtype)
        lambda_s = ns.marginal_lambda(s)
        lambda_0 = ns.marginal_lambda(t_0 * ops.ones_like(s).to(x.dtype))
        h = h_init * ops.ones_like(s).to(x.dtype)
        x_prev = x
        nfe = 0
        if order == 2:
            r1 = 0.5
        ...
        while ops.abs((s - t_0)).mean() > t_err:
            ...
            delta = ops.maximum(ops.ones_like(x).to(x.dtype) * atol, rtol * ops.max(ops.abs(x_lower), ops.abs(x_prev)))
            norm_fn = lambda v: ops.sqrt(ops.square(v.reshape((v.shape[0], -1))).mean(dim=-1, keepdim=True))
            ...
            if ops.all(E <= 1.):
                ...
                x_prev = x_lower
                lambda_s = ns.marginal_lambda(s)
            h = ops.min(theta * h * ops.float_power(E, -1. / order).float(), lambda_0 - lambda_s)
            nfe += order
        print('adaptive solver nfe', nfe)
        return x

    def sample(self, x, steps=10, eps=1e-4, T=None, order=3, skip_type='logSNR',
               adaptive_step_size=False, fast_version=True, atol=0.0078, rtol=0.05, clip_denoised=False
               ):
        """
        ...
        **We recommend DPM-Solver-fast for both fast sampling in few steps (<=20) and fast convergence in many steps (50 to 100).**
        ...
        =====================================================
        Args:
            x: A pyops tensor. The initial value at time `T` (a sample from the normal distribution).
            steps: A `int`. The total number of function evaluations (NFE).
        ...
        if adaptive_step_size:
            ...
            for i, order in enumerate(tqdm(orders)):
                vec_s, vec_t = ops.ones((x.shape[0],)) * timesteps[i], ops.ones((x.shape[0],)) * timesteps[i + 1]
