...
VG_DIR = "/data0/yangzheng/datasets/VG"

parser = argparse.ArgumentParser()

# Input data
parser.add_argument("--splits_json", default="../layout_diffusion/dataset/vg_splits.json")
parser.add_argument("--images_json", default=os.path.join(VG_DIR, "image_data.json"))
parser.add_argument("--objects_json", default=os.path.join(VG_DIR, "objects.json"))
parser.add_argument(
    "--attributes_json", default=os.path.join(VG_DIR, "attributes.json")
)
parser.add_argument(
    "--object_aliases", default=os.path.join(VG_DIR, "object_alias.txt")
)
parser.add_argument(
    "--relationship_aliases", default=os.path.join(VG_DIR, "relationship_alias.txt")
)
parser.add_argument(
    "--relationships_json", default=os.path.join(VG_DIR, "relationships.json")
)
...
def main(args):
    ...
    print("Total imgs number = {}".format(len(list(image_id_to_image.keys()))))
    ...
    print("After filtering there are %d object instances" % len(object_id_to_obj))
    ...
    create_rel_vocab(
        args, train_ids, relationships, object_id_to_obj, rel_aliases, vocab
    )

    print("Encoding objects and relationships ...")
    numpy_arrays = encode_graphs(
        args,
        splits,
        objects,
        relationships,
    ...
    for split_name, split_arrays in numpy_arrays.items():
        ...
        with h5py.File(h5_path, "w") as h5_file:
            ...
            path_dset = h5_file.create_dataset(
                "image_paths", path_shape, dtype=path_dtype
            )
...
def remove_small_images(args, image_id_to_image, splits):
    ...
    for split_name, image_ids in splits.items():
        ...
        print(
            'Removed %d images from split "%s" for being too small'
...
def create_rel_vocab(
    args, image_ids, relationships, object_id_to_obj, rel_aliases, vocab
):
    ...
    for split, image_ids in splits.items():
        ...
        for image_id in image_ids:
            ...
            for obj_attribute in image_id_to_attributes[image_id]:
                obj_id_to_attributes[obj_attribute["object_id"]] = obj_attribute.get(
                    "attributes", None
                )
            ...
            image_wh.append([image_info["width"], image_info["height"]])
