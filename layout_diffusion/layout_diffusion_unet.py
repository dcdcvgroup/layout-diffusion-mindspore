...
class SiLU(nn.Cell):  # export-friendly version of SiLU()
...
class TimestepEmbedSequential(nn.SequentialCell, TimestepBlock):
    ...
    def construct(self, x, emb, cond_kwargs=None):
        extra_output = None
...
class Upsample(nn.Cell):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.dims = dims
        if use_conv:
            self.conv = conv_nd(dims, self.channels, self.out_channels, 3, has_bias=True, padding=1, pad_mode='pad', weight_init='Uniform', bias_init='Uniform')
...
class Downsample(nn.Cell):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        if use_conv:
            self.op = conv_nd(
                dims, self.channels, self.out_channels, 3, stride=stride, has_bias=True, padding=1, pad_mode='pad', weight_init='Uniform', bias_init='Uniform'
            )
        else:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.SequentialCell(
            ...
            SiLU(),
            conv_nd(dims, channels, self.out_channels, 3, has_bias=True, padding=1, pad_mode='pad', weight_init='Uniform', bias_init='Uniform')
        )
        ...
        self.out_layers = nn.SequentialCell(
            ...
            conv_nd(dims, self.out_channels, self.out_channels, 3, has_bias=True, padding=1, pad_mode='pad',
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = conv_nd(
                dims, channels, self.out_channels, 3, has_bias=True, padding=1, pad_mode='pad', weight_init='Uniform', bias_init='Uniform'
            )
        else:
            self.skip_connection = conv_nd(dims, channels, self.out_channels, 1, has_bias=True, weight_init='Uniform', bias_init='Uniform')
    ...
    def construct(self, x, emb):
        ...
        """
        if self.updown:
            in_rest, in_conv = self.in_layers.cell_list[:-1], self.in_layers.cell_list[-1]
            h = x
            for rest in in_rest:
                h = rest(h)
            h = self.h_upd(h)
        ...
        if self.use_scale_shift_norm:
            out_norm, out_rest = self.out_layers.cell_list[0], self.out_layers.cell_list[1:]
            scale, shift = ops.chunk(emb_out, 2, axis=1)  # may cause bug here
            h = out_norm(h) * (1 + scale) + shift
            for rest in out_rest:
                h = rest(h)
        else:
            h = h + emb_out
            h = self.out_layers(h)
    ...
    def __init__(
        ...
        if self.use_positional_embedding:
            self.positional_embedding = ms.Parameter(
                ops.randn(channels // self.num_heads, resolution ** 2) / channels ** 0.5)  # [C,L1]
        else:
        ...
        self.qkv = conv_nd(1, channels, channels * 3, 1, has_bias=True, weight_init='Uniform', bias_init='Uniform')

        self.attention = QKVAttentionLegacy(self.num_heads)

        self.encoder_channels = encoder_channels
        if encoder_channels is not None:
            self.encoder_kv = conv_nd(1, encoder_channels, channels * 2, 1, has_bias=True, weight_init='Uniform', bias_init='Uniform')

        self.proj_out = conv_nd(1, channels, channels, 1, has_bias=True, weight_init='Zero', bias_init='Zero')

    def construct(self, x, cond_kwargs=None):
        '''
        :param x: (N, C, H, W)
        ...
        h = self.proj_out(h)
        output = (x + h).reshape(b, c, height, width)
...
def count_flops_attn(model, _x, y):
    ...
    """
    b, c, height, width = y[0].shape
    num_spatial = int(height * width)
    # We perform two matmuls with the same number of ops.
    # The first computes the weight matrix, the second computes
    # the combination of the value vectors.
    matmul_ops = 2 * b * (num_spatial ** 2) * c
    model.total_ops += ms.Tensor([matmul_ops])
...
class QKVAttentionLegacy(nn.Cell):
    ...
    def construct(self, qkv, encoder_kv=None, positional_embedding=None):
        """
        Apply QKV attention.
        ...
        scale = 1 / np.sqrt(np.sqrt(ch, dtype=qkv.dtype), dtype=qkv.dtype)
        weight = ops.einsum(
            "bct,bcs->bts", q * scale, k * scale
        )  # More stable with f16 than dividing afterwards
        weight = ops.softmax(weight.float(), axis=-1).astype(weight.dtype)
        a = ops.einsum("bts,bcs->bct", weight, v)
...
class ObjectAwareCrossAttention(nn.Cell):
    """
    An attention block that allows spatial positions to attend to each other.
    ...
    def __init__(
        ...
        self.qkv_projector = conv_nd(1, channels, 3 * channels, 1, has_bias=True, weight_init='Uniform', bias_init='Uniform')
        self.norm_for_qkv = normalization(channels)

        if encoder_channels is not None:
            self.layout_content_embedding_projector = conv_nd(1, encoder_channels, channels * 2, 1, has_bias=True, weight_init='Uniform', bias_init='Uniform')
            self.layout_position_embedding_projector = conv_nd(1, encoder_channels, channels, 1, has_bias=True, weight_init='Uniform', bias_init='Uniform')
    ...
    def construct(self, x, cond_kwargs):
        '''
        :param x: (N, C, H, W)
        :param cond_kwargs['xf_out']: (N, C, L2)
        :return:
        ...
        image_patch_positional_embedding = self.norm_for_image_patch_positional_embedding(
            image_patch_positional_embedding)  # (N, C, L1)
        image_patch_positional_embedding = image_patch_positional_embedding.reshape(bs * self.num_heads,
                                                                                    C // self.num_heads,
                                                                                    L1)  # (N // num_heads, C // num_heads, L1)

        # content embedding for image patch
        q_image_patch_content_embedding, k_image_patch_content_embedding, v_image_patch_content_embedding = qkv.split(C,
                                                                                                                      axis=1)  # 3 x (N , C, L1)
        q_image_patch_content_embedding = q_image_patch_content_embedding.reshape(bs * self.num_heads,
                                                                                  C // self.num_heads,
                                                                                  L1)  # (N // num_heads, C // num_heads, L1)
        k_image_patch_content_embedding = k_image_patch_content_embedding.reshape(bs * self.num_heads,
                                                                                  C // self.num_heads,
                                                                                  L1)  # (N // num_heads, C // num_heads, L1)
        v_image_patch_content_embedding = v_image_patch_content_embedding.reshape(bs * self.num_heads,
                                                                                  C // self.num_heads,
                                                                                  L1)  # (N // num_heads, C // num_heads, L1)

        # embedding for image patch
        q_image_patch = ops.cat([q_image_patch_content_embedding, image_patch_positional_embedding],
                                axis=1)  # (N // num_heads, 2 * C // num_heads, L1)
        k_image_patch = ops.cat([k_image_patch_content_embedding, image_patch_positional_embedding],
                                axis=1)  # (N // num_heads, 2 * C // num_heads, L1)
        ...
        layout_positional_embedding = self.layout_position_embedding_projector(
            cond_kwargs['obj_bbox_embedding'])  # N x C x L2
        layout_positional_embedding = self.norm_for_layout_positional_embedding(
            layout_positional_embedding)  # (N, C, L2)
        layout_positional_embedding = layout_positional_embedding.reshape(bs * self.num_heads, C // self.num_heads,
                                                                          L2)  # (N // num_heads, C // num_heads, L2)

        # content embedding for layout
        layout_content_embedding = (cond_kwargs['xf_out'] + self.norm_for_obj_class_embedding(
            cond_kwargs['obj_class_embedding'])) / 2
        k_layout_content_embedding, v_layout_content_embedding = self.layout_content_embedding_projector(
            layout_content_embedding).split(C, axis=1)  # 2 x (N x C x L2)
        k_layout_content_embedding = k_layout_content_embedding.reshape(bs * self.num_heads, C // self.num_heads,
                                                                        L2)  # (N // num_heads, C // num_heads, L2)
        v_layout_content_embedding = v_layout_content_embedding.reshape(bs * self.num_heads, C // self.num_heads,
                                                                        L2)  # (N // num_heads, C // num_heads, L2)

        # embedding for layout
        k_layout = ops.cat([k_layout_content_embedding, layout_positional_embedding],
                           axis=1)  # (N // num_heads, 2 * C // num_heads, L2)
        v_layout = v_layout_content_embedding  # (N // num_heads, C // num_heads, L2)

        #  mix embedding for cross attention
        k_mix = ops.cat([k_image_patch, k_layout], axis=2)  # (N // num_heads, 2 * C // num_heads, L1+L2)
        v_mix = ops.cat([v_image_patch, v_layout], axis=2)  # (N // num_heads, 1 * C // num_heads, L1+L2)
        ...
        key_padding_mask = ops.cat(
            [
                ops.zeros((bs, L1)).bool(),  # (N, L1)
                cond_kwargs['key_padding_mask']  # (N, L2)
            ],
            axis=1
        )  # (N, L1+L2)

        scale = 1 / np.sqrt(np.sqrt(2 * C // self.num_heads, dtype=x.dtype), dtype=x.dtype)
        attn_output_weights = ops.einsum(
        ...
        attn_output_weights = ops.softmax(attn_output_weights.float(), axis=-1).to(
            attn_output_weights.dtype)  # (N x num_heads, L1, L1+L2)

        attn_output = ops.einsum("bts,bcs->bct", attn_output_weights, v_mix)  # (N x num_heads, C // num_heads, L1)
...
class LayoutDiffusionUNetModel(nn.Cell):
    """
    ...
    def __init__(
        ...
        if self.attention_block_type == 'ObjectAwareCrossAttention':
        ...
        self.input_blocks = nn.CellList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, has_bias=True, padding=1, pad_mode='pad', weight_init='Uniform', bias_init='Uniform'))]
        )
        ...
        self.out = nn.SequentialCell(
            normalization(ch),
            SiLU(),
            conv_nd(dims, input_ch, out_channels, 3, has_bias=True, padding=1, pad_mode='pad', weight_init='Zero', bias_init='Zero'),
        )
    ...
    def construct(self, x, timesteps, obj_class=None, obj_bbox=None, is_valid_obj=None, obj_mask=None):
        hs, extra_outputs = [], []
