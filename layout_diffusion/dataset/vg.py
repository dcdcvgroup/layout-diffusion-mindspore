...
class VgSceneGraphDataset:
    ...
    def check_with_relation(self, image_index):
        """
        :param image_index: the idxs of objects of image
        :return: with_relations = [True, False, ....], shape=(O,), O is the number of objects
        """
        obj_idxs = range(self.data['objects_per_image'][image_index])
        with_relations = [False for i in obj_idxs]
        for r_idx in range(self.data['relationships_per_image'][image_index]):
            s = self.data['relationship_subjects'][image_index, r_idx]
            o = self.data['relationship_objects'][image_index, r_idx]
    ...
    def __len__(self):
        # num = self.data['object_names'].size
    ...
    def __getitem__(self, index):
        ...
        obj_class = self.data['object_names'][index][:num_obj]
        ...
        obj_bbox_with_relations = obj_bbox[is_valid_obj & with_relations].astype(float)
        obj_bbox_without_relations = obj_bbox[is_valid_obj & without_relations].astype(float)
        obj_class_with_relations = obj_class[is_valid_obj & with_relations].astype(int)
        obj_class_without_relations = obj_class[is_valid_obj & without_relations].astype(int)
...
#             all_meta_data[key] = ops.stack(value)
#
