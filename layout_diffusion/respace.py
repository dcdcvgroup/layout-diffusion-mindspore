import numpy as np
from mindspore import ops, Tensor, jit_class
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def p_mean_variance(self, model, *args, **kwargs):
        return super().p_mean_variance(self.wrap_model(model), *args, **kwargs)

    def condition_mean(self, cond_fn, *args, **kwargs):
        return super().condition_mean(self.wrap_model(cond_fn), *args, **kwargs)

    def condition_score(self, cond_fn, *args, **kwargs):
        return super().condition_score(self.wrap_model(cond_fn), *args, **kwargs)

    def wrap_model(self, model):
        if isinstance(model, WrappedModel):
            return model
        return WrappedModel(model, self.timestep_map, self.original_num_steps)
